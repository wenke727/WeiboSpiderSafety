...
# 通过sklearn中的TruncatedSVD进行LSA（潜在语义分析，Latent Semantic Analysis）操作，从大段文本/大量文档中抽取主题关键词(topic keywords) 
...
#分词和去停用词处理
sentence_pro = [' '.join([w.strip() for w in jieba.lcut(s) if w not in my_stopwords]) for s in [docs] if len(s) >1]
